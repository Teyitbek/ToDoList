...
public struct ScaledFont {
    ...
    public init(fontName: String) {
        if let url = Bundle.module.url(forResource: fontName, withExtension: "plist"),
           let data = try? Data(contentsOf: url) {
            let decoder = PropertyListDecoder()
            styleDictionary = try? decoder.decode(StyleDictionary.self, from: data)
            styleDictionary?.forEach { registerFont(named: $0.value.fontName, familyName: fontName) }
        }
    }
    ...
    public func font(forTextStyle textStyle: UIFont.TextStyle, fontSize: CGFloat? = nil) -> UIFont {
        guard let styleKey = StyleKey(textStyle),
              let fontDescription = styleDictionary?[styleKey.rawValue],
              let font = UIFont(name: fontDescription.fontName, size: fontSize ?? fontDescription.fontSize)
        else {
    ...
    func registerFont(named name: String, familyName: String) {
        guard !UIFont.fontNames(forFamilyName: familyName).contains(name),
