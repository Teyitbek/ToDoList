...
public class InputVisibilityController: NSObject {
    ...
    open var extraSpaceAboveKeyboard: CGFloat = 20
    ...
    func moveViewUp(up: Bool, usingKeyboardNotification notification: Notification) {
        ...
        if !self.disableKeyboardMoveUpAnimation {
            if let duration = userInfo[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double,
               let animationValue = userInfo[UIResponder.keyboardAnimationCurveUserInfoKey] as? Int,
               let animationCurve = UIView.AnimationCurve(rawValue: animationValue) {
                
                UIView.animate(withDuration: duration,
                               delay: 0,
                               options: animationCurve.toOptions(),
                               animations: {
        ...
        if let scrollView = toMoveView as? UIScrollView {
            ...
            scrollView.__horizontalScrollIndicatorInsets.bottom = insetBottom
...
public extension UIView {
    ...
    private struct AssociatedKeys {
        static var keyboardVisibilityController: StaticString = "keyboardVisibilityController"
    }
    
    var keyboardVisibilityController: InputVisibilityController? {
        get {
            return objc_getAssociatedObject(self, &AssociatedKeys.keyboardVisibilityController) as? InputVisibilityController
        }
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.keyboardVisibilityController, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    
    @discardableResult
    func addInputVisibilityController() -> InputVisibilityController {
        if keyboardVisibilityController == nil {
            let controller = InputVisibilityController(viewToMove: self)
            controller.dismissKeyboardTouchRecognizer = TouchRecognizer(target: controller, action: #selector(InputVisibilityController.dismissFirstResponder))
            controller.dismissKeyboardTouchRecognizer?.ignoreFirstResponder = true
            controller.dismissKeyboardTouchRecognizer?.isEnabled = false
            
            if let recognizer = controller.dismissKeyboardTouchRecognizer {
...
fileprivate extension UIScrollView {
    ...
    var __verticalScrollIndicatorInsets: UIEdgeInsets {
        get { return __contentInset(from: verticalScrollIndicatorInsets) }
        set { verticalScrollIndicatorInsets = __contentInset(from: newValue) }
    }

    var __horizontalScrollIndicatorInsets: UIEdgeInsets {
        get { return __contentInset(from: horizontalScrollIndicatorInsets) }
        set { horizontalScrollIndicatorInsets = __contentInset(from: newValue) }
    }
}
...
extension UIView.AnimationCurve {
    func toOptions() -> UIView.AnimationOptions {
        ...
        case .linear:
            return .curveLinear
        @unknown default:
            return .curveEaseInOut // default fallback
        }
    }
}
